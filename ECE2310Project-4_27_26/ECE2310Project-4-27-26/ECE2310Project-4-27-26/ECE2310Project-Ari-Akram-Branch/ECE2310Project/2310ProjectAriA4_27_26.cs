...
namespace ECE2310Project
{
    public partial class Form : System.Windows.Forms.Form
    {
        ...
        // New runtime combo boxes for recurrence selection (Create / Edit)
        private ComboBox comboBoxRecurring;
        private ComboBox comboBoxEditRecurring;
        ...
        public Form()
        {
            ...
            // create recurrence comboboxes and insert into existing GroupBoxes created by the designer
            CreateRecurrenceComboBoxes();

            // correct day and event arrays
            ...
            RecurrenceType defaultRecurrence = RecurrenceType.Yearly;
            var addRecurring = new RecurringEvent("New Year's Day", year, 1, 1) { RecurrenceType = defaultRecurrence };
            recurringEvents.Add(addRecurring);
            recurringEvents.Add(new RecurringEvent("New Year's Eve", year, 12, 31) { RecurrenceType = defaultRecurrence });
            recurringEvents.Add(new RecurringEvent("Independence Day", year, 7, 4) { RecurrenceType = defaultRecurrence });
            recurringEvents.Add(new RecurringEvent("Christmas", year, 12, 25) { RecurrenceType = defaultRecurrence });
            recurringEvents.Add(new RecurringEvent("Halloween", year, 10, 31) { RecurrenceType = defaultRecurrence });
            recurringEvents.Add(new RecurringEvent("Valentine's Day", year, 2, 14) { RecurrenceType = defaultRecurrence });
            recurringEvents.Add(new RecurringEvent("Juneteenth", year, 6, 19) { RecurrenceType = defaultRecurrence });

            BuildCalendar(drawCalendar);
            BuildExtraStudentFeatures();
        }

        // Create recurrence combo boxes programmatically and add them to the property group boxes
        private void CreateRecurrenceComboBoxes()
        {
            // Create main create-event recurrence combobox
            comboBoxRecurring = new ComboBox();
            comboBoxRecurring.DropDownStyle = ComboBoxStyle.DropDownList;
            comboBoxRecurring.Items.AddRange(new object[] { "Once", "Daily", "Weekly", "Monthly", "Yearly" });
            comboBoxRecurring.SelectedIndex = 0;
            comboBoxRecurring.Name = "comboBoxRecurring";
            comboBoxRecurring.Size = new Size(150, 24);
            comboBoxRecurring.Location = new Point(6, 22); // aligns where the previous checkbox was
            if (groupBoxProperties != null)
            {
                // remove/hide old checkbox if present
                try { if (checkBoxRecurringEvent != null) groupBoxProperties.Controls.Remove(checkBoxRecurringEvent); } catch { }
                groupBoxProperties.Controls.Add(comboBoxRecurring);
            }

            // Create edit-event recurrence combobox
            comboBoxEditRecurring = new ComboBox();
            comboBoxEditRecurring.DropDownStyle = ComboBoxStyle.DropDownList;
            comboBoxEditRecurring.Items.AddRange(new object[] { "Once", "Daily", "Weekly", "Monthly", "Yearly" });
            comboBoxEditRecurring.SelectedIndex = 0;
            comboBoxEditRecurring.Name = "comboBoxEditRecurring";
            comboBoxEditRecurring.Size = new Size(150, 24);
            comboBoxEditRecurring.Location = new Point(6, 22);
            if (groupBoxEditProperties != null)
            {
                try { if (checkBoxEditRecurringEvent != null) groupBoxEditProperties.Controls.Remove(checkBoxEditRecurringEvent); } catch { }
                groupBoxEditProperties.Controls.Add(comboBoxEditRecurring);
            }
        }

        public void BuildCalendar(DrawCalendar dCal)
        {
            // Prepare calendar numbers and reset events text
            ...
            // Compute the visible date range for the 42 calendar cells
            DateTime viewStart = dCal.DateWindow.AddDays(-dCal.StartOfMonth).Date; // first cell date
            DateTime viewEnd = viewStart.AddDays(41).Date; // last cell date

            // Helper to append event text into cell by date
            void AppendToCell(DateTime date, string text)
            {
                int index = (int)(date.Date - viewStart).TotalDays;
                if (index >= 0 && index < 42)
                {
                    if (!string.IsNullOrEmpty(labelEvent[index].Text))
                        labelEvent[index].Text += text + "\n";
                    else
                        labelEvent[index].Text = text + "\n";
                }
            }

            // Render recurring events: generate occurrences inside view window and append them
            for (int i = 0; i < recurringEvents.Count; i++)
            {
                var rec = recurringEvents[i];
                // generate occurrences within view range
                foreach (var occ in RecurrenceHelper.GenerateOccurrences(rec.DateInfo, rec.RecurrenceType, viewStart, viewEnd, rec.RecurrenceInterval, rec.RecurrenceEnd))
                {
                    AppendToCell(occ, rec.Name);
                }
            }

            // Render one-time events: map event date to cell if within view range
            for (int i = 0; i < events.Count; i++)
            {
                var ev = events[i];
                DateTime evDate = ev.DateInfo.Date;
                if (evDate >= viewStart && evDate <= viewEnd)
                {
                    AppendToCell(evDate, ev.Name);
                }
            }
        }
        ...
        private void UpdateList()
        {
            ...
            for (int i = 0; i < eventTracker.Count; i++)
            {
                ...
                // Find a recurring series with this name
                RecurringEvent foundRec = null;
                for (int r = 0; r < recurringEvents.Count; r++)
                {
                    if (recurringEvents[r].Name == eventTracker[i].Name)
                    {
                        foundRec = recurringEvents[r];
                        break;
                    }
                }

                DateTime displayDate = eventTracker[i].DateInfo;
                string recurrenceLabel = "";

                if (foundRec != null)
                {
                    // compute next upcoming occurrence (including today)
                    DateTime searchStart = DateTime.Now.Date;
                    DateTime searchEnd = DateTime.Now.AddYears(5); // reasonable future horizon
                    var next = RecurrenceHelper.GenerateOccurrences(foundRec.DateInfo, foundRec.RecurrenceType, searchStart, searchEnd, foundRec.RecurrenceInterval, foundRec.RecurrenceEnd)
                                .FirstOrDefault();
                    ...
                    recurrenceLabel = foundRec.RecurrenceType != RecurrenceType.Once ? foundRec.RecurrenceType.ToString() : "";
                }
                else
                {
                    // non-recurring: use RecurrenceType on tracked event if set
                    if (eventTracker[i].RecurrenceType != RecurrenceType.Once)
                    {
                        recurrenceLabel = eventTracker[i].RecurrenceType.ToString();
                    }
                }

                if (!string.IsNullOrEmpty(recurrenceLabel))
                {
                    displayName += " (" + recurrenceLabel + ")";
                }

                listBoxEvents.Items.Add(displayName + " on " + displayDate.ToShortDateString());
            }
        ...
        private void AddEvent()
        {
            ...
            // Determine selected recurrence from combobox
            string selectedRecurrenceText = (comboBoxRecurring != null && comboBoxRecurring.SelectedItem != null) ? comboBoxRecurring.SelectedItem.ToString() : "Once";
            RecurrenceType selectedRecurrence = MapStringToRecurrenceType(selectedRecurrenceText);
            ...
            if (selectedRecurrence != RecurrenceType.Once)
            {
            ...
            textBoxEventName.Text = "";
            textBoxEventDecription.Text = "";
            if (comboBoxRecurring != null) comboBoxRecurring.SelectedIndex = 0;
            UpdateList();
            UpdateStats();
        }

        // Helper: map combobox string to enum (safe)
        private RecurrenceType MapStringToRecurrenceType(string s)
        {
        ...
        private void LoadEventIntoEditor(int index)
        {
            ...
            // set recurrence combobox in editor to saved value
            if (comboBoxEditRecurring != null)
            {
                var value = selectedEvent.RecurrenceType.ToString();
                if (comboBoxEditRecurring.Items.Contains(value))
                {
                    comboBoxEditRecurring.SelectedItem = value;
                }
                else
                {
                    comboBoxEditRecurring.SelectedIndex = 0;
            ...
            bool isRecurring = selectedEvent.RecurrenceType != RecurrenceType.Once;
        ...
        private void SaveEditedEvent()
        {
            ...
            bool oldWasRecurring = oldTrackedEvent.RecurrenceType != RecurrenceType.Once;
            ...
            if (oldWasRecurring)
            {
                for (int i = 0; i < recurringEvents.Count; i++)
                {
                    if (recurringEvents[i].Name == oldTrackedEvent.Name &&
                        DateTime.Compare(recurringEvents[i].DateInfo, oldTrackedEvent.DateInfo) == 0)
                    {
                        oldRecurringIndex = i;
                        break;
                    }
            ...
            // new recurrence selected
            string newRecurrenceText = (comboBoxEditRecurring != null && comboBoxEditRecurring.SelectedItem != null) ? comboBoxEditRecurring.SelectedItem.ToString() : "Once";
            RecurrenceType newRecurrence = MapStringToRecurrenceType(newRecurrenceText);
            bool newIsRecurring = newRecurrence != RecurrenceType.Once;
            ...
            eventTracker[editingEventIndex].RecurrenceType = newRecurrence;

            if (oldWasRecurring && newIsRecurring)
            {
                if (oldRecurringIndex >= 0)
                {
                    ...
                    recurringEvents[oldRecurringIndex].RecurrenceType = newRecurrence;
                }
            }
            else if (!oldWasRecurring && !newIsRecurring)
            {
                if (oldEventIndex >= 0)
                {
                    ...
                    events[oldEventIndex].RecurrenceType = newRecurrence;
                }
            }
            else if (oldWasRecurring && !newIsRecurring)
            {
